...
\begin{frame}{分布数据}
  ...
  \begin{figure}[h!]
    ...
    \begin{adjustbox}{max totalsize = {0.60\textwidth}{1.00\textheight}, center}
	  \input{tikz-in-beamer/distributed-data-beamer}
    \end{adjustbox}
  \end{figure}

  \only<1-3>{应用数据:} \uncover<4->{ \textcolor{red}{分布数据 \term{distributed data}:}}
  \begin{enumerate}
...
\begin{frame}{分布共享数据服务}
  \fignocaption{width = 0.50\textwidth}{figures/sina-weibo-redis.pdf}
