...
\begin{frame}{多处理器系统领域中的数据一致性理论}
  ...
  \begin{description}
	\item[一致性:] 追求``相对于 SC {\small (Sequential Consistency)}''的正确性~\footnote{术语称为 SCNF: Sequential Consistency Normal Form.} \citeinbeamer{Adve}{ISCA}{90}
	\item[权衡:] 易编程性 vs. 性能
  \end{description}
...
%%%%%%%%%%%%%%%
\begin{frame}{分布式系统领域中的数据一致性理论}
  \answer{更复杂的权衡\only<1-3>{:} \uncover<4->{$\Longrightarrow$ 一致性理论不追求``相对于 SC''的正确性}}
