...
%%%%%%%%%%%%%%%
\begin{frame}{分布式系统领域中的数据一致性理论}
  \answer{更复杂的权衡 $\Longrightarrow$ 一致性理论不追求``相对于 SC''的正确性}

  \vspace{0.30cm}
  \begin{cdef}[Eventual Consistency]
	\begin{quote}
	  ``\textcolor{red}{If} clients stop issuing update requests, then the replicas
	  will \textcolor{red}{eventually} reach a consistent state.'' \hfill --- \citeinbeamer{Terry}{SOSP}{95} \citeinbeamer{Vogels}{CACM}{09}
	\end{quote}
  \end{cdef}

  \pause

  \fignocaption{width = 0.35\textwidth}{figures/amazon-dynamo-logo.jpg}
  \pause
  \fignocaption{width = 0.50\textwidth, frame}{figures/understanding-eventual-consistency.png}
\end{frame}
%%%%%%%%%%%%%%%
\begin{frame}{分布式系统领域中的数据一致性理论}
...
%%%%%%%%%%%%%%%
\begin{frame}{分布式系统领域中的数据一致性理论}
  \answer{更丰富的应用 $\Longrightarrow$ 一致性理论应体现应用的多样性}
  \vspace{0.50cm}

  % 棒球赛应用一致性需求~\citeinbeamer{Terry}{SOSP}{13}:
  % \begin{description}
  %   \item[记分员:] 采用 \textcolor{blue}{\texttt{\small read-my-writes consistency}} 更新比赛得分
  %   \item[电台记者:] 采用 \textcolor{blue}{\texttt{\small monotonic reads consistency}} 避免
  %   \item[体育新闻记者:] 
  %   \item[统计检查员:] 采用 \textcolor{blue}{\small eventual consistency}
  % \end{description}
  ...
  \begin{table}
	...
	\end{tabular}
	\caption{棒球赛参与者对一致性的不同需求~\citeinbeamer{Terry}{SOSP}{13}.}
  \end{table}
\end{frame}
%%%%%%%%%%%%%%%
