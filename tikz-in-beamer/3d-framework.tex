...
\uncover<2->{
  \draw[thick, >=Stealth, ->] (O) to node[sloped, below, near end, align = center] {\texttt{Mechanism}\\{\tiny (机制: 怎么做)}} (x);
  \draw[thick, >=Stealth, ->] (O) to node[anchor = north, sloped, below, near end, align = center] {\texttt{Measurement}\\{\tiny (度量: 怎么样)}} (y);
  \draw[thick, >=Stealth, ->] (O) to (z) node[anchor = south, align = center] {\texttt{Semantics}\\{\tiny (模型: 是什么)}};
}
